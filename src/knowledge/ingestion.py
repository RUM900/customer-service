    ...
    def remove_document(self, filename: str) -> bool:
        ...
        try:
            if self.vector_store and self.vector_store._collection:
                ids_to_delete = [
                    f"{filename}_chunk_{i:04d}"
                    for i in range(10000)  # 安全上限
                ]
                # 分批查询存在性后删除
                existing = self.vector_store._collection.get(ids=ids_to_delete[:100])
                if existing and existing["ids"]:
                    self.vector_store._collection.delete(ids=existing["ids"])
                logger.info(f"已从索引移除: {filename}")
        except Exception as e:
